
{
    ...
    sub get_element_by_name {
        my $self   = shift;
        my ($name) = @_;
        my $found  = $self->_get_element_by_name(
            sub {
                if ( !defined( $_->name ) ) { return 0 }
                $_->name eq $name;
            }
        );
        return $found;
    ...
    sub get_element_by_lookup {
        my $self     = shift;
        my ($lookup) = @_;
        my $found    = $self->_get_element_by_lookup(
            sub {
                if ( ref($_) ne 'Database::Accessor::Element' ) { return 0 }
                $_->_lookup_name eq $lookup;
            }
        );
        return $found;
    }
...
around BUILDARGS => sub {
    ...
    if ($@) {   # if (exists($ENV{'DA_ALL_ERRORS'}) and $ENV{'DA_ALL_ERRORS'} ){
                # die $@;
                # }
                # else {
        die _one_error( $@, $ops, 'new', $package, $filename, $line,
            $subroutine, 1 );
...
private_method _loadDADClassesFromDir => sub {
    ...
    opendir( DIR, $path )
      or die
"Database::Accessor BUILD Error: Unable to open $path: $! during load of DADs";
    ...
    for (@files) {
        ...
        if ( -d $_ ) {
            ...
            if ($package) {
                ...
                $classname = join '::', 'Database', 'Accessor', 'Driver', $file;
            }
...
has no_retrieve => (
    ...
    description => {
        message =>
"Database::Accessor retrieve Error: Attempt to use retrieve with no_retrieve flag on!",
        not_in_DAD => 1
    }
);
has no_update => (
...
private_method _clean_up_container => sub {
   ...
   # or ($self->view()->alias() and ($field->view ne $self->view()->alias()))));
    ...
    confess( $message .=
            "The \$container must have at least 1 element with the view="
          . $self->view()->name()
          . "!" )
...
private_method _create_or_update => sub {
    ...
    if ( ref($container) eq "ARRAY" ) {
        ...
        if ( scalar(@bad) ) {
            my $count = scalar(@bad);
            $message .=
" The \$container 'Array-Ref' must contain only Hash-refs or Classes. Scalar value";
            $message .= ( $count <= 1 ) ? " " : "s ";
            $message .= join( ',', @bad );
            $message .= ( $count <= 1 ) ? " is" : " are";
            $message .= " not allowed in =" . Dumper($container);
            confess($message);
        }
...
"The \$container parameter must be either a Hash-Ref, a Class or an Array-ref of Hash-refs and or Classes . \$container="
        ...
        $new_container =
          shift( @{ $self->_clean_up_container( $message, [$container] ) } );
    }
...
sub retrieve {
    ...
    confess(
"Database::Accessor retrieve Error: You must supply a da_result_class when da_result_set is Class!"
...
private_method _need_condition => sub {
    ...
    confess(
"Database::Accessor $action Error: Attempt to $action without a condition!"
      )
      if ( $is_required
        and ( $self->condition_count() + $self->dynamic_condition_count() <= 0 )
      );
};
...
private_method _check_options => sub {
    ...
    my ( $action, $opt ) = @_;
    confess(
"Database::Accessor $action Error: The Option param for $action must be a Hash-Ref"
    ) if ( ref($opt) ne 'HASH' );

    foreach my $key ( keys( %{$opt} ) ) {
        confess(
"Database::Accessor $action Error: The $key option param for $action must be a "
    ...
    if ( ref($element) eq 'Database::Accessor::Element' ) {
        ...
        confess("Database::Accessor $type Error: element {name=>"
          ...
          if (
            (
                $type eq 'dynamic'
                and !$self->get_element_by_lookup( $element->_lookup_name() )
            )
          );
        ...
        $element->predicates->condition( $self->default_condition() )
          if ( $self->_add_condition >= 2
            and !$element->predicates->condition() );
        ...
        return
          unless (
            does_role( $element, "Database::Accessor::Roles::Comparators" ) );
...
private_method _get_dad_elements => sub {
    ...
    @elements = @{ $gather->view_elements() }
      if (  ref($gather)
        and $gather->view_elements()
        and $gather->view_count() );
    ...
    for ( my $index = 0 ; $index < scalar(@elements) ; $index++ ) {
        ...
        next
          if (
            !$self->only_elements_is_empty()
            and (
                !$self->only_elements_exists( $element->name )
                and !(
                        $element->alias()
...
private_method _execute => sub {
    ...
    $self->_reset_conditions();
    my @items;
    push( @items, @{ $self->dynamic_conditions }, @{ $self->dynamic_sorts }, );
    ...
    my $gather = undef;
    if ( $action eq Database::Accessor::Constants::RETRIEVE
        and ( $self->gather() or $self->dynamic_gather() ) )
    {
    ...
    my $dad = $driver->new(
        {
            ...
            conditions =>
              [ @{ $self->conditions }, @{ $self->dynamic_conditions } ],
...

{
    ...
    has left => (
        is => 'rw',
        isa =>
'If|Expression|Param|Element|Function|ArrayRefofParams|ArrayRefofElements|ArrayRefofExpressions',
        required      => 1,
        coerce        => 1,
        documentation => "conditions"
    );
...
}
{
    ...
    use Moose::Role;
    with(
        qw(Database::Accessor::Roles::Alias
          )
    );
...
Database::Accessor::Driver that will then act on the provided CRUD methods.
...
| Perl        |  +-+  | |   `---------+     +-----+    +-----------+
| script      |  |A|  |D|   +-----------+   +-------------+
| using       |--|D|--|A|---| DAD Mongo |---| Mongo Engine|
| DA          |  |L|  | |   +-----------+   +-------------+
| Abstraction |  +-+  | |   +---------------+
+-------------+       | |---| Other drivers |-->>
                      +-+   +---------------+

The ADL, or Abstracted Data Language is the kley to working woith DA.  Hopefully,
 it is a easy to undestand and easy to work with set of hash keys that are 
 explained below;

View
Corrosponds to a table in RDBMS or a collection in non-sql DBs. 
The view has the following attributes
name
alias

You can have only one view on a DA

Elements

Is a collection in the form of an array ref of elements that roughly translate to 
fields of a RDBMS table or Keys in a non-sql collection.

The elements array ref can contain any of the following enitties

Element
has the following attribures
name
alias
identity 
no_create
...
only_retrieve
Expression
As the name implies this is an numeric expresion with a left and right side
.  It has the folloing attribures

expression
a numeric operator.  At present DA supports
left
The left side of the expression it can be any one of
element, array of elements, or function params

right
the right side of the expression it can be any one of

open parens
opens a parentathes for you expression
close parne
Closes the parenthese for your expression
Function
a numbeic ,string or aggrigate function of some form. presenigly DA supposrts 

it has the following attribures
left

If
the if then else exprssions.  Many DBs and even non-sql DBs suppor if then else 
expressions so do does DA.  It concists of the following attributes

ifs
and array ref of if elements
...
then
Prarma
use to pass values to the DB
value
Corro



 DADDLAPI, or Application Programming Interface, are the four CRUD functions 
...
   my $da=Database::Accesosr->new($hash);
...
If set to 'TRUE' it will cause 'Create' and 'Update' methods to die when the 
'$container' param does not have either a 'key-value' pair or 'attribute' for 
each of the Element in the DA's Elements attribute.  Elements who's 'view' does 
not match the DA's 'view' are excluded.

available_drivers  Array-Ref

Returns an ArrayRef of HasRefs the DADs that are installed. The keys in the HashRef are 'DAD=>DAD name,class=>the DB class,ver=>the DAD Version'

condtions Array-Ref Read-Only
...
da_result_class
da_result_set
da_supress_view_name
da_warning
default_condition
default_operator
delete_requires_condition
dynamic_conditions
dynamic_condition_count
dynamic_gather
dynamic_gather_count
dynamic_links
dynamic_link_count
dynamic_sorts
dynamic_sort_count
...
no_update
no_delete
only_elements_is_empty
only_elements_exists
retrieve_only
sorts
